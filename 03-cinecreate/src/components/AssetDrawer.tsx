...
async function saveToLibrary(projectId: string, file: File) {
  const blobId = uid();
  await db.blobs.save(blobId, file);
  const meta = await db.meta.get(`assetlib_${projectId}`);
  const list = Array.isArray(meta) ? meta : [];
  list.push({ id: uid(), name: file.name, category: '参考', blobId, createdAt: new Date().toISOString() });
      ...
      for (const item of items) {
        for (const t of item.types) {
          if (t.startsWith('image/')) {
            ...
            await saveToLibrary(projectId, new File([blob], `paste-${Date.now()}.png`, {type: t}));
          }
        }
      }
    } catch {}
  ...
  return (
    <>
      <button className="fixed bottom-5 right-5 z-[100] w-10 h-10 rounded-full bg-white border border-[#e8e5e0] shadow-md flex items-center justify-center text-lg hover:shadow-lg hover:scale-105 transition-all"
        onClick={() => setOpen(!open)} title="素材">📦</button>
      ...
      {open && (
        <div className="fixed bottom-16 right-5 z-[99] bg-white border border-[#e8e5e0] rounded-2xl shadow-xl py-1.5 min-w-[130px] flex flex-col">
          <input ref={fileRef} type="file" accept="image/*,video/*" multiple className="hidden"
            onChange={async e => {
              if (e.target.files?.length && projectId) {
                for (const f of Array.from(e.target.files)) await saveToLibrary(projectId, f);
              }
          ...
          <button className="flex items-center gap-3 px-4 py-2.5 text-[12px] text-[#666] hover:bg-[#f5f3ef] transition-colors text-left"
            onClick={() => { setOpen(false); onOpenPanel(); }}>
            <span className="text-sm w-5 text-center">🗂</span><span>素材库</span>
          </button>
          <button className="flex items-center gap-3 px-4 py-2.5 text-[12px] text-[#999] hover:bg-[#f5f3ef] transition-colors text-left"
