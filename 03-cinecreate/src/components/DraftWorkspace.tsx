    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-[38%] min-w-[280px] shrink-0 border-l border-[var(--border)] overflow-y-auto p-3 space-y-2 bg-[var(--surface)]">
          <div className="flex items-center gap-2 mb-1 px-1"><span className="text-xs text-[var(--dim)] font-semibold">资产库</span><div className="flex-1" /><div className="flex gap-0.5">{[1,2,3,4].map(i => (<div key={i} className={`w-2 h-2 rounded-full ${i < step ? 'bg-[var(--accent-solid)]' : i === step ? 'bg-[#D6B36A]' : 'bg-white'}`} />))}</div></div>
