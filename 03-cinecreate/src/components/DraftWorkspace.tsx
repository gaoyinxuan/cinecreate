      ...
      <div className="flex-1 flex overflow-hidden">
        <div className="flex-1 flex flex-col min-w-0">
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            {/* Messages */}
            {msgs.map((m, i) => (<div key={i} className={`flex ${m.role==='user'?'justify-end':'justify-start'}`}><div className={`max-w-[85%] rounded-2xl px-4 py-3 text-sm ${m.role==='user'?'bg-[#D6B36A] text-white':'bg-[var(--card)] text-[var(--text)] border border-[var(--border)]'}`}><div dangerouslySetInnerHTML={{__html: renderMd(m.content.replace(/<json>[\s\S]*?<\/json>/gi, '').replace(/```json[\s\S]*?```/gi, '').replace(/```[\s\S]*?```/g, '').trim())}} />{!loading && m.role === 'assistant' && i === msgs.length-1 && i > 0 && !m.content.startsWith('📌') && !m.content.startsWith('✅') && !!extractJSON(m.content) && (savedIdx === i ? <div className="mt-2 text-center text-xs text-[var(--accent-text)]/60">✓ 已保存到资产库</div> : <div className="mt-3 pt-3 border-t border-[var(--border)]"><div className="flex justify-center"><button className="px-4 py-1.5 bg-[#D6B36A] hover:bg-[#C49A3C] text-white text-xs font-semibold rounded-lg transition-colors" onClick={() => confirmAsset(i)}>✓ 确认保存到资产库</button></div></div>)}</div></div>))}
            {loading && <div className="text-center text-xs text-[var(--muted)] animate-pulse">AI 思考中...</div>}<div ref={chatEnd} />
          </div>
          {step < 4 && !loading && msgs.length > 1 && (<div className="px-4 py-2 flex justify-center"><button className="px-4 py-1.5 text-xs text-[var(--accent-text)] hover:text-white hover:bg-[#D6B36A] rounded-lg border-2 border-[var(--accent-text)]/30 transition-colors" onClick={advancePhase}>进入下一阶段：{PHASES[step]} →</button></div>)}
          <div className="p-3 border-t border-[var(--border)]"><div className="relative"><textarea className="w-full bg-[var(--card2)] border border-[var(--border2)] rounded-xl px-4 py-3 pr-12 text-sm text-[var(--text)] outline-none focus:border-gold-400 resize-none" style={{height: input.length > 200 ? '120px' : input.length > 80 ? '84px' : '76px', transition: 'height 0.2s'}} placeholder={step===1?'描述你的创意、故事想法或参考作品...\n例如：末日爆发后，机器人骑着鸵鸟穿越废土':step===2?'描述角色需求，或修改已生成的角色设定...':step===3?'描述场景需求，或调整分镜规划...':'描述镜头需求，或调整 Prompt 细节...'} value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>{ if(e.key==='Enter'&&!e.shiftKey){ e.preventDefault(); handleSend(); }}} /><button className={`absolute right-2.5 w-7 h-7 rounded-full flex items-center justify-center transition-all ${loading?'bg-[var(--card2)] text-[var(--muted)]':'bg-[#D6B36A] hover:bg-[#C49A3C] text-white'}`} style={{bottom:24}} disabled={loading} onClick={handleSend} title="发送">
...
function ExpandedAsset({ item, arr, isEditing, editText, setEditText, saveAsset, startEditAsset, optimizePrompt, setEditingAsset, charExpanded, setCharExpanded, doCopy, copied, editStoryData, setEditStoryData, saveStoryEdit, editSceneData, setEditSceneData, saveSceneEdit, buildFullPrompt, assets }: any) {
  ...
  return (<div className="border-t border-[var(--border)] p-3 space-y-3">
    ...
    : (<div className="space-y-2"><p className="text-xs text-[var(--muted)]">暂无内容 · {item.hint}</p><button className="text-xs px-2 py-1 bg-[#D6B36A]/15 hover:bg-[#D6B36A]/25 text-[var(--text)] rounded" onClick={() => startEditAsset(item.key)}>✎ 手动创建</button></div>)}
  </div>);
}
