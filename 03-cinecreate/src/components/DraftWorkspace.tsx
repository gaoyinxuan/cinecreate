                    ...
                    <div key={row.label} className="flex items-center gap-2 mb-2">
                      ...
                      <div className="flex gap-1.5 flex-wrap">
                        {row.tags.map(t => {
                          ...
                          return (
                            <button key={t} className={`text-xs px-2.5 py-1 rounded-full transition-colors ${sel ? 'bg-[var(--accent-solid)] text-white' : 'bg-[var(--card)] border border-[var(--border)] text-[var(--text3)] hover:text-[var(--text)] hover:border-[var(--accent-text)]/30'}`}
...
function ExpandedAsset({ item, arr, isEditing, editText, setEditText, saveAsset, startEditAsset, optimizePrompt, setEditingAsset, charExpanded, setCharExpanded, doCopy, copied, editStoryData, setEditStoryData, saveStoryEdit, editSceneData, setEditSceneData, saveSceneEdit, buildFullPrompt, assets }: any) {
  if (editStoryData) { const s = editStoryData; return (<div className="border-t border-[var(--border)] p-3 space-y-2"><div className="text-xs text-[var(--muted)] font-semibold mb-2">编辑故事设定</div><input className="w-full bg-[var(--surface)] border border-[var(--border)] rounded px-2 py-1.5 text-sm text-[var(--text)] outline-none focus:border-accent-400" placeholder="故事名称" value={s.title||''} onChange={(e:any) => setEditStoryData({...s, title: e.target.value})} /><div className="grid grid-cols-2 gap-2"><input className="bg-[var(--surface)] border border-[var(--border)] rounded px-2 py-1.5 text-xs text-[var(--text)] outline-none focus:border-accent-400" placeholder="视频时长" value={s.duration||''} onChange={(e:any) => setEditStoryData({...s, duration: e.target.value})} /><select className="bg-[var(--surface)] border border-[var(--border)] rounded px-2 py-1.5 text-xs text-[var(--text)] outline-none focus:border-accent-400" value={s.visualTone?.medium||''} onChange={(e:any) => setEditStoryData({...s, visualTone: {...s.visualTone, medium: e.target.value}})}><option value="">视觉媒介...</option><option>超写实电影</option><option>真人电影</option><option>影视级CG</option><option>日漫</option><option>美漫</option><option>二次元动画</option><option>吉卜力风格</option><option>新海诚风格</option><option>厚涂插画</option><option>游戏CG</option></select></div><input className="w-full bg-[var(--surface)] border border-[var(--border)] rounded px-2 py-1.5 text-xs text-[var(--text)] outline-none focus:border-accent-400" placeholder="参考作品（1部）" value={s.visualTone?.reference||''} onChange={(e:any) => setEditStoryData({...s, visualTone: {...s.visualTone, reference: e.target.value}})} /><textarea className="w-full bg-[var(--surface)] border border-[var(--border)] rounded px-2 py-1.5 text-xs text-[var(--text)] outline-none focus:border-accent-400 resize-none" rows={3} placeholder="世界观设定" value={s.worldBuilding||''} onChange={(e:any) => setEditStoryData({...s, worldBuilding: e.target.value})} /><textarea className="w-full bg-[var(--surface)] border border-[var(--accent-text)]/15 rounded px-2 py-1.5 text-xs text-[var(--text)] outline-none focus:border-accent-400 resize-none" rows={3} placeholder="故事简介（≤200字）" value={s.summary||''} onChange={(e:any) => setEditStoryData({...s, summary: e.target.value})} /><div className="flex gap-2 pt-2"><button className="flex-1 text-xs py-1.5 bg-[var(--accent-solid)] hover:bg-[var(--accent-hover)] text-white rounded" onClick={saveStoryEdit}>保存修改</button><button className="text-xs px-3 py-1.5 text-[var(--muted)] hover:text-[var(--text2)] rounded" onClick={() => setEditStoryData(null)}>取消</button></div></div>); }
  if (editSceneData) { const s = editSceneData.data; const shots: any[] = editSceneData.shots || []; const totalSec = shots.reduce((sum:number, sh:any) => sum + (parseInt(sh.duration)||0), 0); const updateShot = (i: number, field: string, val: any) => { const ns = [...shots]; ns[i] = {...ns[i], [field]: val}; setEditSceneData({...editSceneData, shots: ns}); }; const addShot = () => { setEditSceneData({...editSceneData, shots: [...shots, {shotNumber: shots.length+1, duration:'5s', shotType:'中景', purpose:'', transition:''}]}); }; const delShot = (i: number) => { setEditSceneData({...editSceneData, shots: shots.filter((_:any,si:number) => si !== i).map((sh:any,si:number) => ({...sh, shotNumber: si+1}))}); }; return (<div className="border-t border-[var(--border)] p-3 space-y-2"><div className="text-xs text-[var(--muted)] font-semibold">编辑场景</div><input className="w-full bg-[var(--surface)] border border-[var(--border)] rounded px-2 py-1.5 text-sm text-[var(--text)] outline-none focus:border-accent-400" placeholder="场景名称" value={s.name||''} onChange={(e:any) => setEditSceneData({...editSceneData, data: {...s, name: e.target.value}})} /><textarea className="w-full bg-[var(--surface)] border border-[var(--border)] rounded px-2 py-1.5 text-xs text-[var(--text)] outline-none focus:border-accent-400 resize-none" rows={3} placeholder="场景描述（含环境、氛围、镜头语言）" value={s.description||''} onChange={(e:any) => setEditSceneData({...editSceneData, data: {...s, description: e.target.value}})} /><div className="text-xs text-[var(--muted)] font-semibold pt-1">分镜列表</div>{shots.map((sh: any, si: number) => (<div key={si} className="flex items-center gap-1"><span className="text-xs text-[var(--accent-text)] font-mono w-10">Shot{sh.shotNumber}</span><input className="w-12 bg-[var(--surface)] border border-[var(--border)] rounded px-1 py-1 text-xs text-[var(--text)] outline-none focus:border-accent-400" placeholder="时长" value={sh.duration||''} onChange={(e:any) => updateShot(si, 'duration', e.target.value)} /><input className="w-14 bg-[var(--surface)] border border-[var(--border)] rounded px-1 py-1 text-xs text-[var(--text)] outline-none focus:border-accent-400" placeholder="景别" value={sh.shotType||''} onChange={(e:any) => updateShot(si, 'shotType', e.target.value)} /><input className="flex-1 bg-[var(--surface)] border border-[var(--border)] rounded px-1 py-1 text-xs text-[var(--text)] outline-none focus:border-accent-400" placeholder="作用" value={sh.purpose||''} onChange={(e:any) => updateShot(si, 'purpose', e.target.value)} /><input className="w-16 bg-[var(--surface)] border border-[var(--border)] rounded px-1 py-1 text-xs text-[var(--text)] outline-none focus:border-accent-400" placeholder="转场" value={sh.transition||''} onChange={(e:any) => updateShot(si, 'transition', e.target.value)} /><button className="text-xs text-[var(--muted)] hover:text-red-400 shrink-0" onClick={() => delShot(si)}>✕</button></div>))}<button className="text-xs px-2 py-1 text-[var(--muted)] hover:text-[var(--text)] rounded border border-[var(--border)] w-full" onClick={addShot}>+ 添加镜头</button><div className="flex gap-2 pt-2"><button className="flex-1 text-xs py-1.5 bg-[var(--accent-solid)] hover:bg-[var(--accent-hover)] text-white rounded" onClick={saveSceneEdit}>保存修改</button><button className="text-xs px-3 py-1.5 text-[var(--muted)] hover:text-[var(--text2)] rounded" onClick={() => setEditSceneData(null)}>取消</button></div></div>); }
  if (isEditing) { return (<div className="border-t border-[var(--border)] p-3 space-y-3"><textarea className="w-full bg-[var(--surface)] border border-[var(--border)] rounded-lg p-2 text-xs text-[var(--text2)] font-mono outline-none focus:border-accent-400 resize-none" rows={8} value={editText} onChange={(e:any) => setEditText(e.target.value)} /><div className="flex gap-2"><button className="flex-1 text-xs px-2 py-1 bg-[var(--accent-solid)] hover:bg-[var(--accent-hover)] text-white rounded" onClick={saveAsset}>保存</button><button className="text-xs px-2 py-1 text-[var(--muted)] hover:text-[var(--text2)] rounded" onClick={() => setEditingAsset(null)}>取消</button></div></div>); }
