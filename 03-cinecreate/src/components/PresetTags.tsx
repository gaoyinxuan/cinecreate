...
export default function PresetTags({ tags, onChange }: Props) {
  ...
  return (
    <div className="relative" onMouseLeave={() => setOpen(false)}>
      <button className={`text-xs px-2 py-1 rounded flex items-center gap-1 max-w-[200px] ${count ? 'bg-indigo-500/15 text-[var(--text)] border border-indigo-500/20' : 'text-[var(--muted)] hover:text-[var(--text3)] border border-transparent hover:border-[var(--border2)]'}`}
            ...
            <div key={key} className="mb-2 last:mb-0">
              ...
              <div className="flex flex-wrap gap-1">
                {g.opts.map(opt => {
                  ...
                  return <button key={opt} className={`text-xs px-2 py-1 rounded-full ${active ? 'bg-indigo-500 text-white' : 'bg-[var(--card2)] text-[var(--dim)] hover:text-[var(--text2)] hover:bg-[#1a1a32]'}`}
