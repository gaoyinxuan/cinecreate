      ...
      <div className="px-4 py-3 border-b border-[var(--border)]">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div className="text-[13px] font-semibold text-[var(--text)] leading-tight">CineCreate <span className="text-[10px] text-[var(--muted)] font-medium ml-1">影创</span></div>
          </div>
              ...
              <div className="hidden group-hover:flex items-center relative shrink-0" onClick={e=>e.stopPropagation()}>
                ...
                {menuOpen===prj.id && (
                  <div className="absolute right-0 top-full mt-1 bg-[var(--card)] border border-[var(--border)] rounded-md shadow-lg z-50 py-0.5 min-w-[90px]">
                    <button className="block w-full text-left text-[11px] px-3 py-1 text-[var(--text2)] hover:bg-black/5"
              ...
              <div className="ml-5 border-l border-[var(--border)] pl-2 space-y-0">
                ...
                <div>
                  ...
                  {showDocs[prj.id] && <div className="ml-3">
                    ...
                    {drafts.map((d:any)=>(
                      <div key={d.id} className={`${rowBase} text-[11px] px-1 ${selectedDraftId===d.id?'text-[var(--text)] font-medium bg-black/5':'text-[var(--text3)] hover:text-[var(--text2)] hover:bg-black/2'}`}
                ...
                <div>
                  ...
                  {showTool[prj.id] && <div className="ml-3">
                    <div className={`${rowBase} text-[11px] px-1 ${activeMode==='tools-image'?'text-[var(--text)] font-medium bg-black/5':'text-[var(--text3)] hover:text-[var(--text2)] hover:bg-black/2'}`}
                      onClick={()=>onSelectImageTools()}><span className="w-3.5 shrink-0" /><span className="flex-1">生图</span></div>
                    <div className={`${rowBase} text-[11px] px-1 ${activeMode==='tools-video'?'text-[var(--text)] font-medium bg-black/5':'text-[var(--text3)] hover:text-[var(--text2)] hover:bg-black/2'}`}
