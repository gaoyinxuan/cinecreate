...
export default function SequenceTabs({ sequences, activeSeqId, onSelect, onCreate, onRename, onDelete, shotsBySeq }: Props) {
  ...
  return (
    <div className="flex items-center gap-1 px-6 py-2.5 bg-[var(--bg)] border-b border-[var(--border)] overflow-x-auto">
      <button className={`shrink-0 text-xs px-3 py-1.5 rounded-lg transition-colors ${!activeSeqId ? 'bg-indigo-500/20 text-[var(--text)] border border-indigo-500/30' : 'text-[var(--dim)] hover:text-[var(--text2)] border border-transparent hover:border-[var(--border2)]'}`}
      ...
      {(sequences||[]).map(s => {
        ...
        return (
          <div key={s.id}
            className={`shrink-0 group flex items-center gap-1 text-xs px-3 py-1.5 rounded-lg cursor-pointer transition-colors ${active ? 'bg-indigo-500/15 text-[var(--text)] border border-indigo-500/30' : 'text-[var(--text3)] hover:text-[var(--text2)] border border-transparent hover:border-[var(--border2)]'}`}
            onClick={() => onSelect(s.id)} onDoubleClick={() => startRename(s)}>
            {editId === s.id ? (
              <input className="bg-[var(--card2)] border border-indigo-500 rounded px-1 py-0 text-xs text-[var(--text)] outline-none w-20"
      ...
      <button className="shrink-0 text-[var(--muted)] hover:text-indigo-400 text-lg px-2" onClick={() => setShowCreate(true)}>+</button>
