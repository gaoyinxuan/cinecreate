...
export default function ShotBoard({ project, shots, shotGlobalNum, onChangeShot, onDeleteShot, onAddImages, onAddImagesToShot, navigatorRef, activeSeqId }: Props) {
  ...
  const dropZone = (msg: string) => (
    <div className="border-2 border-dashed border-[var(--border2)] hover:border-indigo-500/50 rounded-xl p-12 text-center cursor-pointer"
  ...
  return (
    <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 bg-[var(--bg)]">
      {!activeSeqId ? (
        shots.length === 0 ? (
          ...
          <div className="max-w-4xl mx-auto space-y-4 pb-16">
            {(shots||[]).map((s, i) => <ShotCard key={s.id} shot={s} globalNum={shotGlobalNum[s.id]} onChange={onChangeShot} onDelete={() => onDeleteShot(s.id)} />)}
          </div>
        )
