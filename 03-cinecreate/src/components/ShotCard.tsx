  ...
  return (
    <div id={`shot-${shot.id}`} className="bg-[var(--card)] border border-[var(--border2)] rounded-xl overflow-hidden hover:border-[var(--border2)] transition-colors group">
      <div className="flex items-center gap-2.5 px-4 py-2.5 border-b border-white/5">
        ...
        {durationSec !== null && <span className="text-xs text-indigo-400 font-mono ml-1">⏱ {Math.floor(durationSec/60)}m {durationSec%60}s</span>}
      ...
      <div className="flex gap-4 p-4">
        ...
        <div className="flex-1 flex flex-col gap-3 min-w-0">
          <input className="bg-transparent text-base font-semibold text-[var(--text)] outline-none border-b border-transparent focus:border-indigo-500 pb-1" placeholder="分镜标题" value={shot.title} onChange={e => field('title')(e.target.value)} />
          <textarea className="flex-1 bg-[var(--card2)] border border-[var(--border2)] rounded-lg p-3 text-sm text-[var(--text)] outline-none resize-none focus:border-indigo-500 min-h-[80px]" placeholder="分镜描述、旁白文案、镜头说明..." value={shot.description} onChange={e => field('description')(e.target.value)} />
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {durationSec !== null && <span className="text-xs text-indigo-400 font-mono font-semibold">{Math.floor(durationSec/60)}m {durationSec%60}s</span>}
