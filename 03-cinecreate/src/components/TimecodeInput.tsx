...
export default function TimecodeInput({ value, onChange, placeholder = '00:00', hasError }: Props) {
  ...
  return (
    <input className={`bg-[var(--card2)] border rounded px-2 py-1 text-[var(--text)] w-20 text-center font-mono text-xs focus:outline-none ${hasError ? 'border-red-500/50' : 'border-[var(--border2)] focus:border-accent-400'}`}
