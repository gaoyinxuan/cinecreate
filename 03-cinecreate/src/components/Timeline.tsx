        ...
        {(groups||[]).map(group => {
          ...
          return (<div key={group.seqId} className="mb-1">
            ...
            {(!activeSeqId && seq && isCollapsed) ? null : (group.shots||[]).map((s, li) => {
              ...
              return (
                <div key={s.id}
                  className={`relative flex items-center gap-2 px-2 py-1.5 rounded-lg text-xs transition-all duration-200 ${isActive?'bg-indigo-500/15 border border-indigo-500/20':'hover:bg-white/[0.04] border border-transparent'} ${isDragging?'tl-dragging':''} ${landedId===s.id?'tl-landed':''}`}
                  ...
                  <span className={`shrink-0 w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold ${isActive?'bg-indigo-500 text-white':'bg-[var(--border)] text-[var(--dim)]'}`}>{shotGlobalNum[s.id] || li+1}</span>
