  ...
  return (
    <div className="border-b border-[var(--border)] p-4">
      <div className="flex items-center gap-2 mb-3">
        ...
        <button className="text-xs px-2 py-1 text-[var(--text3)] hover:text-indigo-400 hover:bg-white/[0.04] rounded" onClick={()=>fileRef.current?.click()}>+ 添加片段</button>
      ...
      {!collapsed && segments.length === 0 && (
        <div className="border-2 border-dashed border-[var(--border2)] hover:border-indigo-500/50 rounded-xl p-8 text-center cursor-pointer"
            ...
            {isExp && (
              <div className="flex gap-4 mt-2">
                ...
                <div className="flex-1 flex flex-col gap-2 text-xs">
                  <div className="flex items-center gap-2"><span className="text-[var(--dim)] w-12">名称</span><input className="flex-1 bg-[var(--card2)] border border-[var(--border2)] rounded px-2 py-1 text-[var(--text)] outline-none focus:border-indigo-500" value={seg.name} onChange={e=>onUpdate({...sequence,videoSegments:segments.map(x=>x.id===seg.id?{...x,name:e.target.value}:x)})} /></div>
                  ...
                  <div className="flex items-start gap-2"><span className="text-[var(--dim)] w-12 shrink-0 pt-1">备注</span><textarea className="flex-1 bg-[var(--card2)] border border-[var(--border2)] rounded px-2 py-1 text-[var(--text)] outline-none focus:border-indigo-500 resize-none h-10" placeholder="版本备注..." value={seg.versionNote} onChange={e=>onUpdate({...sequence,videoSegments:segments.map(x=>x.id===seg.id?{...x,versionNote:e.target.value}:x)})} /></div>
